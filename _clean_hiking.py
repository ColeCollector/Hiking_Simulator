...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:  # If the user closes the window
            ...
            if game_status == 'game':
                sounds[1].play()
                speed = (780-max(feet[0][1],feet[1][1]))/15
                if locked != -1:  # If a foot is locked
                    pos = adjust_position_if_in_forbidden_circle(pos)
    ...
    if game_status == 'menu':
        ...
        if bgcolor != colors[biome] and (score/50 > lastbiomeswitch + 27):
        ...
        if score/50 > biomeswitch:
            ...
            biomeswitch += random.randint(30,40)
        ...
        if stamina <= 0 or feet[0][1] - 20 > HEIGHT or feet[1][1] - 20 > HEIGHT:   
            transition = min(60, transition + 1)
            dead += 1
        ...
        pygame.draw.circle(transition_surf, (255, 255, 255), (screen.get_width() // 2,screen.get_height() // 2), (60 - abs(transition)) * 8)
